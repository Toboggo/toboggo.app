...
export async function fetchWeather(lat: number, lng: number): Promise<WeatherReading> {
  ...
  // Non-UI marker: the mobile weather query has retry:false and never surfaces
  // this error (the chip/alert just don't render).
...
// The weather-alert copy now lives in the app's `map` namespace
// (`map:weather.*`) — the mobile MapExplore builds it via i18next.
